...
1. Вы через процесс ffmpeg отправляете в неё видеофайл с камеры или жесткого диска:
...
2. MediaMTX принимает этот поток и раздает его по адресу `rtsp://localhost:8554/live`

3. Ваше приложение на FastAPI подключается к этому адресу, чтобы забирать кадры и отправлять их в нейросеть (MobileNetV3).